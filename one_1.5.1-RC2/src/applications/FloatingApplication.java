...
public class FloatingApplication extends Application {
    ...
    public Message handle(Message msg, DTNHost host) {
        ...
        if (type == null || !type.equalsIgnoreCase("floating")) {
            return msg; // Not a floating message
        }
    ...
    public void update(DTNHost host) {
	    ...
	    */
	    /* ttl: ttl, .., ttl_max in ttl_gran steps */
            double msgttl = (ttl_max == ttl)? ttl :
                    (rng_ttl.nextInt(((int) ttl_max - (int) ttl) / (int) ttl_gran + 1)) * ttl_gran + ttl;
	    
	    /* anchors: a/r, ..., a_max/r_max in a_gran/r_gran steps*/
            double r = (anchor_r_max == anchor_r)? anchor_r :
                    (rng_r.nextInt(((int) anchor_r_max - (int) anchor_r) / (int) anchor_r_gran + 1)) * anchor_r_gran +
                    anchor_r;
            double a = r;  /* default behavior: no buffer zone */
            double anchor_a_min = 0;

	    /* size: 100, 200, ..., 1000 KB */
            int msgsize = (msgSizeMax == msgSize)? msgSize :
                    (rng_size.nextInt((msgSizeMax - msgSize) / msgSizeGran + 1)) * msgSizeGran + msgSize;
		    ...
		    /* pick a random value between MAX (r, anchor_a) and anchor_a_max */
                    ...
                    a = (rng_a.nextInt(((int) anchor_a_max - (int) anchor_a_min) / (int) anchor_a_gran + 1)) *
                        anchor_a_gran + anchor_a_min;
                    if (a < r) {
                        a = r;
                    }
                }
