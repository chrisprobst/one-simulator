    ...
    public void update() {
        ...
        if (location_source == LOC_SRC_NONE) {
            ...
            for (Connection c : connections) {
                if (((FloatingContentRouter) c.getOtherNode(getHost()).getRouter()).getLocationSource() ==
                    LOC_SRC_GPS) {
        ...
        if (deletion_policy == DEL_ENCOUNTER && location_valid) {
            ...
            for (Connection c : new_conns) {
                for (Message m : m_set) {
                    distance_curr = loc.distance((Coord) m.getProperty(FC_ANCHOR));
                    if ((deletion_check(distance_curr, (Double) m.getProperty(FC_R), (Double) m.getProperty(FC_A)) ==
                         1)) {
