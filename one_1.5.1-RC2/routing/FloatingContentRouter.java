...
/**
 * Replication policy options fifo: do nothing special; replicate in the order in which they are found in the buffer rnd: shuffle randomly saf: smallest anchor
 * zone (area) first svf[2]: smallest cylinder (area x volume) first; svf2 uses area, svf just radius stf[2]: smallest total (area x volume x ttl) first; stf2
 * uses area, stf just radius
 */
...
public class FloatingContentRouter extends ActiveRouter {
		...
		if (fcSettings.contains(FC_LOCATION_SOURCE)) {
			if (fcSettings.getSetting(FC_LOCATION_SOURCE).equals("gps")) {
				...
				if (fcSettings.contains(FC_LOCATION_UPDATE_INTERVAL)) {
					if (!fcSettings.getSetting(FC_LOCATION_UPDATE_INTERVAL).equals("none") && !fcSettings.getSetting(FC_LOCATION_UPDATE_INTERVAL).equals("0")) {
	...
	public boolean createNewMessage(Message msg) {
		/*
		 * we record the absolute position of the node when posting the message, since the perceived position is already in the anchor point.
		 */
	...
	/*
	 * Some functions should be carried out only once per encounter. Record the new encounters here for later use in update()
	 */
	@Override
	...
	public void update() {
		...
		/*
		 * If we do not have an own notion of a location, we infer our location as the mean of the nodes we are connected to. This can be done in reality if the
		 * other nodes broadcast beacons with their location information.
		 */
		...
		if (deletion_policy == DEL_ENCOUNTER && location_valid) {
			/*
			 * This branch of the code deletes the message upon first encounter of another node outside the anchor zone We loop through the new connections to
			 * perform the evaluation of the probabilistic deletion check once per new connection.
			 */
			for (@SuppressWarnings("unused")
			Connection c : new_conns) {
				for (Message m : m_set) {
					distance_curr = loc.distance((Coord) m.getProperty(FC_ANCHOR));
		...
		for (Message m : m_ordered_list) {
			/*
			 * find the right connection(s) for each message Messages are replicated to nodes if the target node is within the anchor zone If the other node
			 * doesn't know where it is, we'll pick the position of the local node. If the local node doesn't know its position either, we check if the peer
			 * knows its previous location and choose this. If this isn't known either, we stay passive and don't do anything.
			 * 
			 * We prefer our most recent position over the peer's because our location was surely established in this round and may this be more accurate.
			 */
	...
	public Coord getLocationWithError(double min, double max, double interval, int seed) {
		...
		if (gpsLocation.getX() == -1 || SimClock.getTime() >= gpsLastReading + gpsUpdateInterval) {
			...
			if (max > 0) {
				...
				double d = this.rdDistance.nextGaussian() * (gpsErrorMax - gpsErrorMin) / 6 + (gpsErrorMax - gpsErrorMin);

				gpsLocation.setLocation(location.getX() + Math.cos(angle) * d, location.getY() + Math.sin(angle) * d);
			} else {
