...
/**
 * Superclass of active routers. Contains convenience methods (e.g. {@link #getNextMessageToRemove(boolean)}) and watching of sending connections (see {@link #update()}).
 * 
 * This class now extends the extended MessageRouter instead of the simple MessageRouter, nothing else was (and should be) changed in this class.
 * 
 * @author Aalto University, ComNet, Andre Ippisch
 */
public abstract class ActiveRouter extends MessageRouterExtended {
	/**
	 * Delete delivered messages -setting id ({@value} ). Boolean valued. If set to true and final recipient of a message rejects it because it already has it, the message is deleted from buffer. Default=false.
	 */
	...
	/**
	 * Called when a connection's state changes. If energy modeling is enabled, and a new connection is created to this node, reduces the energy for the device discovery (scan response) amount
	 * 
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		/**
		 * N.B. With application support the following if-block becomes obsolete, and the response size should be configured to zero.
		 */
	 ...
	 * @return A return code similar to {@link MessageRouter#receiveMessage(Message, DTNHost)}, i.e. {@link MessageRouter#RCV_OK} if receiving seems to be OK, TRY_LATER_BUSY if router is transferring, DENIED_OLD if the router is already
	 *         carrying the message or it has been delivered to this router (as final recipient), or DENIED_NO_SPACE if the message does not fit into buffer
	 */
	 ...
	 * 
	 * @param size
	 *            Size of the new message transferred, the transfer is aborted before message is removed
	...
	/**
	 * Tries to make room for a new message. Current implementation simply calls {@link #makeRoomForMessage(int)} and ignores the return value. Therefore, if the message can't fit into buffer, the buffer is only cleared from messages that
	 * are not being sent.
	 * 
	 * @param size
	 ...
	 * @param tuples
	 *            The tuples to try
	 * @return The tuple whose connection accepted the message or null if none of the connections accepted the message that was meant for them.
	 */
	...
	/**
	 * Tries to send all given messages to all given connections. Connections are first iterated in the order they are in the list and for every connection, the messages are tried in the order they are in the list. Once an accepting
	 * connection is found, no other connections or messages are tried.
	 * 
	 * @param messages
	...
	/**
	 * Tries to send all messages that this router is carrying to all connections this node has. Messages are ordered using the {@link MessageRouter#sortByQueueMode(List)}. See {@link #tryMessagesToConnections(List, List)} for sending
	 * details.
	 * 
	 * @return The connections that started a transfer or null if no connection accepted a message.
	 */
	...
	/**
	 * Exchanges deliverable (to final recipient) messages between this host and all hosts this host is currently connected to. First all messages from this host are checked and then all other hosts are asked for messages to this host. If a
	 * transfer is started, the search ends.
	 * 
	...
	/**
	 * Method is called just before a transfer is aborted at {@link #update()} due connection going down. This happens on the sending host. Subclasses that are interested of the event may want to override this.
	 * 
	 * @param con
	...
	/**
	 * Method is called just before a transfer is finalized at {@link #update()}. Subclasses that are interested of the event may want to override this.
	 * 
	 * @param con
	...
	public RoutingInfo getRoutingInfo() {
		...
		if (energy != null) {
			top.addMoreInfo(new RoutingInfo("Energy level: " + String.format("%.2f mAh", energy.getEnergy() / 3600)));
		}
		return top;
	}
