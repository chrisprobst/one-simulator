...
/**
 * Implementation of PRoPHETv2" router as described in http://tools.ietf.org/html/draft-irtf-dtnrg-prophet-09
 * 
 * This class now extends the extended ActiveRouter instead of the simple MessageRouter, nothing else was (and should be) changed in this class.
 * 
 * @author Aalto University, ComNet, Andre Ippisch
 */
public class ProphetV2Router extends ActiveRouterExtended {
	 ...
	 * 
	 * @param The
	 *            router prototype where setting values are copied from
	 */
	...
	/**
	 * Returns the timestamp of the last encouter of with the host or -1 if entry for the host doesn't exist.
	 * 
	 * @param host
	 *            The host to look the timestamp for
	...
	private void updateTransitivePreds(DTNHost host) {
		...
		assert otherRouter instanceof ProphetV2Router : "PRoPHETv2 only works with other routers of same type";
	...
	/**
	 * Comparator for Message-Connection-Tuples that orders the tuples by their delivery probability by the host on the other side of the connection (GRTRMax)
	 */
	private class TupleComparator implements Comparator<Tuple<Message, Connection>> {
		...
		public int compare(Tuple<Message, Connection> tuple1, Tuple<Message, Connection> tuple2) {
			...
			double p1 = ((ProphetV2Router) tuple1.getValue().getOtherNode(getHost()).getRouter()).getPredFor(tuple1.getKey().getTo());
			// -"- tuple2...
			double p2 = ((ProphetV2Router) tuple2.getValue().getOtherNode(getHost()).getRouter()).getPredFor(tuple2.getKey().getTo());
