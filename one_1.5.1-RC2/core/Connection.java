...
public abstract class Connection {
	...
	/**
	 * Sets a message that this connection is currently transferring. If message passing is controlled by external events, this method is not needed (but then
	 * e.g. {@link #finalizeTransfer()} and {@link #isMessageTransferred()} will not work either). Only a one message at a time can be transferred using one
	 * connection.
	 * 
	 * @param m
	 *            The message
	...
	/**
	 * Clears the message that is currently being transferred. Calls to {@link #getMessage()} will return null after this.
	 */
	...
	/**
	 * Finalizes the transfer of the currently transferred message. The message that was being transferred can <STRONG>not</STRONG> be retrieved from this
	 * connections after calling this method (using {@link #getMessage()}).
	 */
	...
	public String toString() {
		return fromNode + "<->" + toNode + " (" + getSpeed() / 1000 + " kBps) is " + (isUp() ? "up" : "down")
