...
public class Message implements Comparable<Message> {
	/**
	 * Time-to-live (TTL) as seconds -setting id ({@value} ). Boolean valued. If set to true, the TTL is interpreted as seconds instead of minutes.
	 * Default=false.
	 */
	...
	/**
	 * if a response to this message is required, this is the size of the response message (or 0 if no response is requested)
	 */
	...
	/**
	 * Container for generic message properties. Note that all values stored in the properties should be immutable because only a shallow copy of the properties
	 * is made when replicating messages
	 */
	 ...
	 * @param id
	 *            Message identifier (must be unique for message but will be the same for all replicates of the message)
	 * @param size
	...
	/**
	 * Returns the time to live (in minutes or seconds, depending on the setting {@link #TTL_SECONDS_S}) of the message or Integer.MAX_VALUE if the TTL is
	...
	/**
	 * Deep copies message data from other message. If new fields are introduced to this class, most likely they should be copied here too (unless done in
	 * constructor).
	 * 
	 * @param m
	...
	/**
	 * Adds a generic property for this message. The key can be any string but it should be such that no other class accidently uses the same value. The value
	 * can be any object but it's good idea to store only immutable objects because when message is replicated, only a shallow copy of the properties is made.
	 * 
	 * @param key
	...
	/**
	 * Updates a value for an existing property. For storing the value first time, {@link #addProperty(String, Object)} should be used which checks for name
	 * space clashes.
	 * 
	 * @param key
	 *            The key which is used to lookup the value
	...
	public void updateProperty(String key, Object value) throws SimError {
		if (this.properties == null) {
			/*
			 * lazy creation to prevent performance overhead for classes that don't use the property feature
			 */
